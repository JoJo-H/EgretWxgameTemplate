...
class ImageFactory {
    ...
    */
    public getImage(resName: string, resFile: string = null, isCenter: boolean = false): egret.Bitmap {

        let resKey: string = this.getFormatKey(resFile, resName);
        let imageList: egret.Bitmap[] = this.m_caches.get(resKey);
        if (!imageList) {
            imageList = [];
            this.m_caches.add(resKey, imageList);
        }
        if (imageList.length > 0) {
            return imageList.pop();
        } else {
            let bitmap: egret.Bitmap = new egret.Bitmap();
            let texture: egret.Texture = RES.getRes(resKey);
            if (texture) {
                bitmap.texture = texture;
            } else {
                RES.getResAsync(resKey).then((textureData) => {
                    bitmap.texture = textureData;
                })
            }
            if (isCenter) {
    ...
    */
    public getFormatKey(resFile: string, resName: string): string {
        if (!resFile || resFile == 'undefined' || resFile == 'null' || resFile == '') {
            return resName;
        }
        return `${resFile}_${resName}`;
    }
    /**
    * 检查图片是否匹配图集和资源名称
    * @param {string} resFile	图集名称
    ...
    */
    ...
    public getCacheCountByKey(resName: string, resFile: string): number {
        let resKey: string = this.getFormatKey(resFile, resName);
        let imageList: egret.Bitmap[] = this.m_caches.get(resKey);
