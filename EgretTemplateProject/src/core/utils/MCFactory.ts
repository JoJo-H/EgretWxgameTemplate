...
class MCFactory {
    ...
    /**
     * 获取影片剪辑
     * @param  {string} mcFile  影片剪辑json及图集名称
     * @param  {string} mcName  动画名称
     * @param  {boolean=true} isCenter  是否锚点居中
     * @returns egret.MovieClip 影片剪辑对象
     */
    public async getMovieClip(mcFile: string, mcName: string, isCenter: boolean = false): Promise<egret.MovieClip> {
        ...
        if (!factory) {
            let jsonData: any = await RES.getResAsync(`${mcFile}_json`);
            let pngData: egret.Texture = await RES.getResAsync(`${mcFile}_png`);
