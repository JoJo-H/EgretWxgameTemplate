...
class WebUtils {
    ...
    public static addKeyboardListener(validChecker?: (event: KeyboardEvent) => boolean): void {
        document.onkeydown = function (event: KeyboardEvent): any {
            if (event) {
                if (!validChecker || validChecker(event)) {
                    core.EventManager.getInstance().sendEvent(new core.KeyboardEventData(core.EventID.KEYBOARD_DOWN, event));
                }
            }
        }
        document.onkeyup = function (event: KeyboardEvent): any {
            if (event) {
                if (!validChecker || validChecker(event)) {
                    core.EventManager.getInstance().sendEvent(new core.KeyboardEventData(core.EventID.KEYBOARD_UP, event));
                }
            }
        }
    }
        ...
        if (visProp) {
            ...
            document.addEventListener(evtname, function () {
                ...
                if (WebUtils.isHidden()) {
                    core.EventManager.getInstance().sendEvent(new core.EventData(WebUtils.PAGE_HIDE));
                } else {
                    core.EventManager.getInstance().sendEvent(new core.EventData(WebUtils.PAGE_SHOW));
                }
            }, false);
        }
    }
