...
var targetPath="./resource/assets/animation/worker";
var templeJson = JSON.parse(fs.readFileSync('./tool/worker_temp.json','utf8'));
var tempReg = /31024/g;
...
function readDirSync(path){
	...
	pa.forEach(function(ele,index){
		...
		if(info.isDirectory()){
			var mcJson = path+"/"+ele+"/ani_worker_"+ele+".json";
					...
					if(!targetJson.hasOwnProperty("mc")){
						...
						fs.writeFile(mcJson, JSON.stringify(tempMarkJson), {flag: 'a'}, function (err) {
							if(err) {
								...
								console.log('转化成功:'+"ani_worker_"+ele+".json");
							}
						});
					}
					else{
